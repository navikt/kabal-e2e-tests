...
test.describe('Navigering', () => {
  ...
  test('"Oppgaver"-lenke navigerer til `/oppgaver`', async ({ page }) => {
    const behandlingerLink = getMainMenu(page).getByRole('link', { name: 'Oppgaver', exact: true });
  ...
  test('"Mine Oppgaver"-lenke navigerer til `/mineoppgaver`', async ({ page }) => {
    const link = getMainMenu(page).getByRole('link', { name: 'Mine Oppgaver' });
  ...
  test('"Søk"-lenke navigerer til `/sok`', async ({ page }) => {
    const link = getMainMenu(page).getByRole('link', { name: 'Søk på person' });
    await link.click({ timeout });
