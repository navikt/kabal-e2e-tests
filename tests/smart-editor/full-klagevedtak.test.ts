...
test.describe('Smart editor - Klagevedtak', () => {
  test('Smart editor - Klagevedtak', async ({ klagebehandling }) => {
    const { page } = klagebehandling;
    const smartEditor = await klagebehandling.behandling.initSmartEditor('Vedtak/beslutning (klage)');
    ...
    await test.step('Sett inn regelverk', async () => {
      ...
      await regelverk.getByLabel('Oppdater regelverk').click();
    ...
    await test.step('Send ut', async () => {
      await klagebehandling.behandling.finishAndVerifyDocument('Klagevedtak');
    });
  });
});
